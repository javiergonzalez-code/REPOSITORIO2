...
$sugerencias_usuarios = computed(function () {
    ...
    $sugerencias = User::select('name')->where('name', 'like', "%{$this->userFilter}%")->orderBy('name', 'asc')->take(6)->get();
...
$logs = computed(function () {
    ...
    if ($this->esProveedor) { $query->where('user_id', $user->id); }
    if ($this->search) { $query->where('descripcion', 'like', "%{$this->search}%"); }
    if ($this->userFilter && !$this->esProveedor) { $query->whereHas('user', fn($q) => $q->where('name', 'like', "%{$this->userFilter}%")); }
    if ($this->accion) { $query->where('accion', $this->accion); }
    if ($this->fecha) { $query->whereDate('created_at', $this->fecha); }
                ...
                @if(!$this->esProveedor)
                    <div class="col-lg-3 col-md-6">
                        ...
                        <div style="position: relative !important;">
                            ...
                            <input type="text" wire:model.live.debounce.300ms="userFilter" class="form-control ps-5" placeholder="Escribir usuario..." autocomplete="off">
