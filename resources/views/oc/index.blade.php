...
<style>
    ...
    .btn-gradient:hover { transform: translateY(-2px); box-shadow: 0 5px 15px rgba(26, 35, 126, 0.3); color: white; }
                                    ...
                                    <td>
                                        <div class="d-flex align-items-center">
                                            ...
                                            <div class="ms-3">
                                                ...
                                                <div class="text-muted" style="font-size: 0.7rem;">ID: #{{ str_pad($oc->user->id, 4, '0', STR_PAD_LEFT) }}</div>
                                            </div>
                                        </div>
                                    </td>
                                    <td>
                                        <div class="d-flex align-items-center">
                                            ...
                                            @endif
                                            <span class="text-primary fw-semibold small text-break">{{ $oc->nombre_original }}</span>
                                        </div>
                                    </td>
                                    <td class="text-center">
                                        <div class="text-dark fw-bold small">{{ $oc->created_at->format('d/m/Y') }}</div>
                                        <div class="text-muted small" style="font-size: 0.7rem;">{{ $oc->created_at->format('H:i') }} hrs</div>
                                    </td>
                                    <td class="text-end pe-4">
                                        <div class="btn-group rounded-pill overflow-hidden">
                                            <a href="{{ route('oc.preview', $oc->id) }}" class="btn btn-white btn-sm px-3" title="Previsualizar">
                                                <i class="fas fa-eye text-muted"></i>
                                            </a>
                                            <a href="{{ route('oc.download', $oc->id) }}" class="btn btn-white btn-sm px-3" title="Descargar">
